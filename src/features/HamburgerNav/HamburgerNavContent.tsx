        ...
        <div className={"c-hb-nav__content"}>
            <ul className={"c-hb-nav__lists"}>
                ...
                <li className={"hbNavItem"}>
                    ...
                    href="/service"
                    >
                        <span className={"s-ML -s18 -b -ls-2 pdr5 pdr4s"}>マネーリペアの特徴</span>
                    </Link>
                </li>
                <li className={"hbNavItem c-borderBtm -dotted"}>
                    <Link className={`mgb5 mgb5s ${isActive("/interview") ? "-active" : ""}`} href="/interview" >
                        <span className={"s-ML -s18 -b -ls-2 pdr5 pdr4s"}>ご利用者の声</span>
                    </Link>
                    <ul className={"c-flex -col2 -jc-st"}>
                        {interviewCategories.contents.map((category: Category) => (
                            <li key={category.id} className={"hbNavCategoryItem flexItem mgb5 mgb5s"}>
                                <Link
                                    className={`s-M -s14 -b -left pdl7s ${isActive(`/interview/category/${category.id}`)
                                        ? "-active"
                                        : ""
                                    }`} href={`/interview/category/${category.id}`}
                                >
                                    <span className={"s-S -s14 -b -left pdl7s"}>{category.name}</span>
                                </Link>
                            </li>
                        ))}
                    </ul>
                ...
                <li className={"hbNavItem c-borderBtm -dotted"}>
                    <Link
                        className={`mgb5 mgb5s ${isActive("/information") ? "-active" : ""}`} href="/information"
                    >
                        <span className={"s-ML -s18 -b -ls-2 pdr5 pdr4s"}>お役立ち情報</span>
                    </Link>
                    <ul className={"c-flex -col2 -jc-st"}>
                        {informationCategories.contents.map((category: Category) => (
                            <li key={category.id} className={"hbNavCategoryItem flexItem mgb5 mgb5s"}>
                                <Link
                                    ...
                                    isActive(`/information/category/${category.id}`)? "-active" : ""}`}
                                    href={`/information/category/${category.id}`}
                                >
                                    <span className={"s-S -s14 -b -white -left pdl7s"}>{category.name}</span>
                                </Link>
                            </li>
                        ))}
                    </ul>
                ...
                <li className={"hbNavItem"}>
                    ...
                    >
                    <span className={"s-ML -s18 -b -ls-2 pdr5 pdr4s"}>資料ダウンロード</span>
                    </Link>
                </li>
                <li className={"hbNavItem"}>
                    <Link
                    ...
                    >
                    <span className={"s-ML -s18 -b -ls-2 pdr5 pdr4s"}>お問い合わせ</span>
                    </Link>
                </li>
            </ul>
        </div>
